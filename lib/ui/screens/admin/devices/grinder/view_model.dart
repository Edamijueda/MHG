...
class GrinderViewModel extends ReactiveViewModel {
  final log = getStackedLogger('GrinderViewModel');
  ...
  List<Device>? get reactiveGrinder => _fireStoreDbService.reactiveGrinder;
  ...
  Future addDevice() async {
    ...
    DialogResponse? response = await _dialogService.showCustomDialog(
      variant: DialogType.grinderEntry,
      hasImage: true,
      takesInput: true,
    );
    if (response?.data != null) {
      await _cloudStorageService.uploadGrinder(
        collectionPath: grinderDbPathTxt,
  ...
  Future deleteDevice({
    ...
    if (response?.confirmed == true) {
      ...
      try {
        _fireStoreDbService.removeGrinderFromFS(device: device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
      ...
      try {
        _cloudStorageService.removeGrinderFromStorage(device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
  ...
  realtimeOperations() {
    callGrinderRealtimeUpdate();
  }

  void callGrinderRealtimeUpdate() {
    _fireStoreDbService.getGrinderRealtimeUpdate();
  }
}
