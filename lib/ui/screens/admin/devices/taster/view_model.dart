...
class TasterViewModel extends ReactiveViewModel {
  final log = getStackedLogger('TasterViewModel');
  ...
  List<Device>? get reactiveTaster => _fireStoreDbService.reactiveTaster;
  ...
  Future addDevice() async {
    ...
    DialogResponse? response = await _dialogService.showCustomDialog(
      variant: DialogType.tasterEntry,
      hasImage: true,
      takesInput: true,
    );
    if (response?.data != null) {
      await _cloudStorageService.uploadTaster(
        collectionPath: tasterDbPathTxt,
  ...
  Future deleteDevice({
    ...
    if (response?.confirmed == true) {
      ...
      try {
        _fireStoreDbService.removeTasterFromFS(device: device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
      ...
      try {
        _cloudStorageService.removeTasterFromStorage(device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
  ...
  realtimeOperations() {
    callTasterRealtimeUpdate();
  }

  void callTasterRealtimeUpdate() {
    _fireStoreDbService.getTasterRealtimeUpdate();
  }
}
