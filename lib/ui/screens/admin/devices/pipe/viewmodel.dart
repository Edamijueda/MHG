...
class PipeViewModel extends ReactiveViewModel {
  final log = getStackedLogger('PipeViewModel');
  ...
  List<Device>? get reactivePipe => _fireStoreDbService.reactivePipe;
  ...
  Future deleteDevice({
    ...
    if (response?.confirmed == true) {
      ...
      try {
        _fireStoreDbService.removePipeFromFS(device: device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
      ...
      try {
        _cloudStorageService.removePipeFromStorage(device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
  ...
  realtimeOperations() {
    callPipeRealtimeUpdate();
  }

  void callPipeRealtimeUpdate() {
    _fireStoreDbService.getPipeRealtimeUpdate();
  }
}
