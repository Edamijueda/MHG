...
class BongViewModel extends ReactiveViewModel {
  final log = getStackedLogger('BongViewModel');
  ...
  List<Device>? get reactiveBong => _fireStoreDbService.reactiveBong;
  ...
  Future addDevice() async {
    ...
    DialogResponse? response = await _dialogService.showCustomDialog(
      variant: DialogType.bongEntry,
      hasImage: true,
      takesInput: true,
    );
    if (response?.data != null) {
      await _cloudStorageService.uploadBong(
        collectionPath: bongDbPathTxt,
  ...
  Future deleteDevice({
    ...
    if (response?.confirmed == true) {
      ...
      try {
        _fireStoreDbService.removeBongFromFS(device: device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
      ...
      try {
        _cloudStorageService.removeBongFromStorage(device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
  ...
  realtimeOperations() {
    callBongRealtimeUpdate();
  }

  void callBongRealtimeUpdate() {
    _fireStoreDbService.getBongRealtimeUpdate();
  }
}
