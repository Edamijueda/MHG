...
class BubbleViewModel extends ReactiveViewModel {
  final log = getStackedLogger('BubbleViewModel');
  ...
  List<Device>? get reactiveBubble => _fireStoreDbService.reactiveBubble;
  ...
  Future addDevice() async {
    ...
    DialogResponse? response = await _dialogService.showCustomDialog(
      variant: DialogType.bubbleEntry,
      hasImage: true,
      takesInput: true,
    );
    if (response?.data != null) {
      await _cloudStorageService.uploadBubble(
        collectionPath: bubbleDbPathTxt,
  ...
  Future deleteDevice({
    ...
    if (response?.confirmed == true) {
      ...
      try {
        _fireStoreDbService.removeBubbleFromFS(device: device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
  ...
  realtimeOperations() {
    callBubbleRealtimeUpdate();
  }

  void callBubbleRealtimeUpdate() {
    _fireStoreDbService.getBubbleRealtimeUpdate();
  }
}
