...
class FlowerViewModel extends ReactiveViewModel {
  ...
  List<Device>? get reactiveFlower => _fireStoreDbService.reactiveFlower;

  @override
  List<ReactiveServiceMixin> get reactiveServices => [_fireStoreDbService];

  Future chooseFlowerType() async {
    log.i('no param');
  ...
  Future deleteDevice({
    ...
    if (response?.confirmed == true) {
      ...
      try {
        _fireStoreDbService.removeFlowerFromFS(device: device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
      ...
      try {
        _cloudStorageService.removeFlowerFromStorage(device);
      } catch (e) {
        reusableFunction.snackBar(
            message:
  ...
  Future addDevice(String flowerType) async {
    log.i('param flowerType is: $flowerType');
    ...
    if (response?.data != null) {
      await _cloudStorageService.uploadFlower(
          collectionPath: flowerDbPathTxt,
  ...
  realtimeOperations() {
    callFlowerRealtimeUpdate();
  }

  void callFlowerRealtimeUpdate() {
    _fireStoreDbService.getFlowerRealtimeUpdate();
  }
}
